...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 10 == 0:
            ...
            pred_choice = pred.data.max(1)[1]
            correct = pred_choice.eq(target.data).cpu().sum()
            performance.append(correct.item()/float(opt.batchSize * 2500))
    ...
    if performance > best_performance:
        print(f"Saving new model: {performance} > {best_performance}")
        best_performance = performance
        torch.save(classifier.state_dict(), '%s/best_seg_model_%s.pth' % (opt.outf, opt.class_choice))
